...
using ulong_matrix = Eigen::Matrix<std::size_t, Eigen::Dynamic, Eigen::Dynamic,
				  Eigen::RowMajor>;
...
double inline filter_score(Eigen::Ref<matrix> control,
			   ...
			   Eigen::Ref<matrix>& M2, std::size_t i2,
			   std::size_t j2) {
...
void Aligngraph::build_edges_with_filter(Eigen::Ref<matrix>& pts1,
    ...
    auto rule_func = [&control_pts, &mask, &_c, &_rot_con, &_RCU, &percentage](
			 Eigen::Ref<matrix>& p1, std::size_t i1,
			 std::size_t j1, Eigen::Ref<matrix>& p2,
...
void init_Aligngraph(pybind11::module& mm) {
	...
	.def(init<Eigen::Ref<matrix>&, std::size_t, Eigen::Ref<matrix>&,
		  std::size_t>())
