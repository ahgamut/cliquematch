...
{
   public:
    Aligngraph(Eigen::Ref<matrix>& pts1, std::size_t pts1_len, Eigen::Ref<matrix>& pts2,
               std::size_t pts2_len)
        : _A2Agraph(pts1, pts1_len, pts2, pts2_len, euclidean, true, euclidean, true)
    {
    }
    void build_edges_with_filter(Eigen::Ref<matrix>& pts1, Eigen::Ref<matrix>& pts2,
                                 Eigen::Ref<matrix> control_pts,
                                 Eigen::Ref<matmask> mask, double percentage = 0.8);
};
