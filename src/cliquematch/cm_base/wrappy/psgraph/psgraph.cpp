...
void
psgraph::build_edges_with_filter(Eigen::Ref<matrix> controlpts,
                                 Eigen::Ref<matmask> img,
                                 double percentage)
{
    EDGES = edges_filt_pointsets(
      nvert, nedges, S1, S2, controlpts, img, pts_epsilon, percentage);
    if (EDGES.data() == NULL || EDGES.size() == 0) {
        throw std::runtime_error("Could not extract edges!!\n");
    }
    this->G = graph(nvert, nedges, EDGES);
