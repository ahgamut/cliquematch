...
void
graphBits::clear(u32 N)
{
    u32 i = 0;
    if (N == 0 || N == this->valid_len || N / 32 == this->dlen) {
        for (i = 0; i < this->dlen; i++)
            this->data[i] = 0;
    } else {
...
bool
graphBits::block_empty(u32 i) const
{
...
void
graphBits::show(std::vector<u32> elements)
{
...
void
graphBits::show(u32* elements, u32 len)
{
...
std::vector<u32>
graphBits::get_subset(std::vector<u32> elements)
{
    // assert(this->valid_len <= elements.size());
...
std::vector<u32>
graphBits::get_subset(u32* elements, u32 len)
{
    // assert(this->valid_len <= len);
    std::vector<u32> ans(this->count());
    for (u32 i = 0, ct = 0; i < this->valid_len; i++) {
